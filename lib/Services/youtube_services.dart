...
class YouTubeServices {
  ...
  Future<Map?> formatVideoFromId({
    ...
    final String quality = await MyApp.hiveMutex.protect(() async {
      if (!Hive.isBoxOpen('settings')) {
        Hive.init(path);
      }
      await Hive.openBox('settings');
      final res = Hive.box('settings')
          .get(
            'ytQuality',
            defaultValue: 'High',
          )
  ...
  Future<Map?> refreshLink(String id) async {
    ...
    try {
      quality =
          Hive.box('settings').get('quality', defaultValue: 'High').toString();
    } catch (e) {
      quality = 'High';
    }
  ...
  Future<Map?> formatVideo({
    ...
    if (getUrl) {
      // check cache first
      final bool containsKey = await MyApp.hiveMutex.protect(() async {
            await Hive.openBox('ytlinkcache');
            final res = Hive.box('ytlinkcache').containsKey(video.id.value);
            return res;
          }) ??
          false;
      if (containsKey) {
        final Map cachedData = await MyApp.hiveMutex.protect(() async {
          await Hive.openBox('ytlinkcache');
          final res = Hive.box('ytlinkcache').get(video.id.value) as Map;
          return res;
        });
        ...
        if ((DateTime.now().millisecondsSinceEpoch ~/ 1000) + 350 >
          ...
          // giving cache link
          Logger.root.info('video url cache found for ${video.id.value}');
...
/*        final collection = await BoxCollection.open(
          ...
          path:
              pathToUse, // Path where to store your boxes (Only used in Flutter / Dart IO)
        );*/
        MyApp.hiveMutex.protect(() async {
          await Hive.openBox('ytlinkcache');
