...
class Download with ChangeNotifier {
  ...
  bool createDownloadFolder = Hive.box('settings')
      .get('createDownloadFolder', defaultValue: true) as bool;
  ...
  Future<void> prepareDownloadOld(
  ...
  Future<void> prepareDownload(
    ...
    final bool exists = await cacheFile.exists();
    if (exists && Hive.box('downloads').get(data['id'].toString()) != null) {
      Logger.root.info('File already exists');
      lastDownloadId = data['id'].toString();
      notifyListeners();
    } else {
        ...
        try {
          ...
          if (downloadLyrics) {
            ...
            final Map res = await Lyrics.getLyrics(
              id: data['perma_url'].split('/').last.toString(),
