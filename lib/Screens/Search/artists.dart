...
import 'package:soundal/CustomWidgets/artist_like_button.dart';
    ...
    if (!status) {
      ...
      callSpotifyFunction(
        function: (String accessToken) async => {
          SpotifyApi()
              .getArtistDetails(accessToken, widget.data['id'].toString())
              .then((value) async {
            (value['artist'] as Map).forEach((key, val) {
              widget.data[key] = val;
            });
            data = value['sections'] as Map<String, List>;
            ...
            if (data['Top Songs'] != null) {
              ...
              data['Top Songs']?.clear();
              data['Top Songs']?.addAll(songList);

              if (data['Singles'] != null) {
                for (final element in data['Singles']!) {
              ...
              if (data['Related Artists'] != null) {
                for (final element in data['Related Artists']!) {
                  element['image'] = element['images']?.first['url'];
                  element['title'] = element['name'];
                  /*element['artist'] = (element['artists'] as List)
                      .map((e) => e['name'])
                                            ...
                                            ShowSnackBar().showSnackBar(
                                              ...
                                              /*AppLocalizations.of(
                                                context,
                                                  ...
                                                  .connectingRadio,*/
                                              'Radio implementation not ready yet',
                                              duration: const Duration(
                                                seconds: 5,
                                              ),
                                            );
                                            //TODO: spotify recommendations api with artist id
